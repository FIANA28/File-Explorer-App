...
const getMimeType = extension => {
    return new Promise((resolve, reject) => {
        https.get(mimeURL, response => {
            if(response.statudCode < 200 || response.statusCode > 299){
                reject(`Error: Failed to load mime types json file:${resoinse.statusCode}`);
            console.log(`Error: Failed to load mime types json file:${resoinse.statusCode}`);
            return false;
            }
